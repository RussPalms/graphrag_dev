...
from .api import (
    ...
    enable_logging_with_pipeline_config,
    load_progress_reporter,
    run_pipeline,
)
from .cache import NoopPipelineCache
...
def index_cli(
    ...
    if overlay_defaults or config:
        ...
        pipeline_config = create_pipeline_config(default_config, verbose)
    else:
        try:
    ...
    (logging_enabled, log_path) = enable_logging_with_pipeline_config(
        pipeline_config, run_id, verbose
    )

    if not logging_enabled:
        info(
            f"Reports logging not enabled for pipeline config: {_redact(pipeline_config.model_dump())}",
            True,
        )
    else:
    ...
    info(f"Final Pipeline Config: {_redact(pipeline_config.model_dump())}", verbose)
